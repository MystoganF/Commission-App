...
public class PublicService {
    ...
    public List<PublicArtistDto> getPublicArtists() {
        // Filter by the "ADMIN" role since that represents your Portfolio Owners
    ...
    public ArtistFullPortfolioDto getArtistFullDetails(Long artistId) {
        ...
        dto.works = workRepo.findByArtistIdOrderByCreatedAtDesc(artistId).stream()
                .map(w -> {
                    ...
                    res.id = w.getId(); res.title = w.getTitle(); res.imageUrl = w.getImageUrl();
                    res.category = w.getCategory(); res.year = w.getYear(); res.description = w.getDescription();
                    return res;
                }).collect(Collectors.toList());
        ...
        dto.education = educationRepo.findByArtistIdOrderByEndYearDesc(artistId).stream().map(e -> {
            ...
            res.startYear = e.getStartYear(); res.endYear = e.getEndYear();
            return res;
        }).collect(Collectors.toList());
        ...
        dto.achievements = achievementRepo.findByArtistIdOrderByYearDesc(artistId).stream().map(a -> {
            ...
            res.id = a.getId(); res.title = a.getTitle(); res.year = a.getYear(); res.description = a.getDescription();
            return res;
        }).collect(Collectors.toList());
    ...
    public List<PublicServiceDto> getTopRatedServices() {
        ...
        return allServices.stream()
                ...
                .sorted((a, b) -> Double.compare(b.getAverageRating(), a.getAverageRating())) // Highest first
                .limit(6) // Only show top 6 on landing
                .collect(Collectors.toList());
    }

    // ── Helpers for Calculations and Mapping ──
    ...
    private PublicArtistDto toPublicArtistDto(User u) {
        ...
        // Bulletproof Skills (Prevents 500 Error if empty)
    ...
    private PublicServiceDto toPublicServiceDto(ArtistServiceEntity s) {
        ...
        // Prevent LazyLoad crashes on collections
